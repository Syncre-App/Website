    ...
    try {
        ...
        return new Promise<NextResponse>((resolve) => {
            ...
            connection.query(query, [hash, salt], (error, results) => {
                ...
                const friends = results as RowDataPacket[];

                if (friends.length === 0) {
                    resolve(NextResponse.json({ error: 'Friends not found' }, { status: 404 }));
                    return;
                }

                resolve(NextResponse.json({ friends: friends }, { status: 200 }));
            });
        });
    } catch (error) {
